...
try {
  const data = JSON.parse(
    fs.readFileSync(base + "datafiles/db/items.json", "utf8")
  );
...
instances.push("last_created", "purple_guy", "o_player", "o_follower");
fs.readdirSync(base + "sprites")
  .filter((f) => f.includes("npc"))
  .forEach((f) => instances.push(f));

const sprites = fs
  .readdirSync(base + "sprites")
  .filter((f) => !f.startsWith(".")); //.DS_Store go away!!!!
...
async function generateSnippets() {
  ...
  for await (const line of rl) {
    lines.push(
      line
        .replaceAll("%PARTY_CHARACTER%", partyChars)
        ...
        .replaceAll("%ITEM%", items.join(","))
    );
  }

  // Write back the file
  fs.writeFile(
    "snippets/umbrascript.code-snippets",
    lines.join("\r\n"),
    (err) => {
      if (err) {
        console.error(err);
